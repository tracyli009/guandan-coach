...
test('build() output attaches all 11 pure-logic modules to window.GD when module/require are absent (browser simulation)', () => {
  const { build } = require('../build.js');
  const html = build();
  ...
  const expected = ['Knowledge', 'Cards', 'HandOrganizer', 'HandOptimizer', 'Combos', 'Moves', 'Engine', 'AI', 'CoachRealtime', 'CoachReview', 'CoachHistory'];
