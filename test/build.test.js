...
test('build() output attaches all 10 pure-logic modules to window.GD when module/require are absent (browser simulation)', () => {
  const { build } = require('../build.js');
  const html = build();
  ...
  const expected = ['Knowledge', 'Cards', 'HandOrganizer', 'Combos', 'Moves', 'Engine', 'AI', 'CoachRealtime', 'CoachReview', 'CoachHistory'];
